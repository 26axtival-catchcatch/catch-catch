...
if TYPE_CHECKING:
    ...
    from customer_signal.exploration.source_adapter import PagedSourceAdapter, ValidatedSourceSet
...
class UnsupportedPaginationError(ValueError):
    """A selected legacy adapter cannot participate in exploration."""
...
class SourceRegistry:
    ...
    def get_paged(self, source_id: SourceId) -> PagedSourceAdapter:
        """Return a runtime-checked native-page adapter without exposing the registry."""

        from customer_signal.exploration.source_adapter import PagedSourceAdapter
    ...
    def prepare_paged_sources(
        self, scope: ExplorationSourceScope
    ) -> ValidatedSourceSet:
        """Validate native identity page chains and pin their source snapshots."""

        from customer_signal.exploration.contracts import ExplorationSourceScope
        from customer_signal.exploration.source_adapter import prepare_validated_source_set

        if not isinstance(scope, ExplorationSourceScope):
            raise ValueError("exploration scope must be an ExplorationSourceScope")
        return prepare_validated_source_set(self._adapters, scope)
