...
**Architecture:** One candidate per verifier, concurrency three, filtered handoff, recoverable result previews, compact history and a request-content budget. Keep server ownership checks and independent measurements.
...
- [ ] Run investigation, signal and Bedrock regression suites plus ruff and git diff --check. Replay saved synthetic candidates through the new verifier, compare latency/context and decisions against baseline. Rebuild/restart the local backend after passing checks.
- [ ] Record measured results and remaining limitations; retain raw provider content and credentials outside all outputs.
