...
기본 화면은 공식 5색을 사용하고 파생값은 `color-mix`로 만듭니다. 경고 상태와
아래 에이전트 대화의 캐릭터 식별색은 예외입니다.
...
고객의 단서를 함께 찾는 작은 전문가들로 역할을 표현합니다. 캐릭터는 발화자를 쉽게
구분하기 위한 장치이며, 대화 내용과 판단을 새로 만들지 않습니다.

| 이름 | 실제 역할 | 식별 요소 | 캐릭터 색 |
| --- | --- | --- | --- |
| 길잡이 | 조율 에이전트 | 헤드셋과 방향 표시 | 브랜드 핑크 |
| 탐정 | 조사 에이전트 | 돋보기 | 라벤더 `#b5adf1` |
| 체커 | 검증 에이전트 | 안경 | 민트 `#9bd5c4` |
| 기록가 | 정리 에이전트 | 노트와 펜 | 크림 `#e9c896` |

캐릭터 색은 `conversation.module.css` 안에서만 정의합니다. 아바타, 이름, 선택 표시와
발화 그룹의 옅은 배경에 사용하고, 검증 결과나 신뢰도를 뜻하는 색으로 쓰지 않습니다.
실행 중인 역할의 표시만 움직이며 중단·완료·모션 감소 설정에서는 멈춥니다.

발화 옆에는 이름과 실제 역할, 시간, 진행 설명·결과 요약 구분을 함께 표시합니다.
서버가 전달한 문장과 불확실성, 판단 근거를 유지합니다. 의인화를 위해 대사나 반응을
추가하거나, 검증 역할의 모든 발화를 검증 통과로 표시하지 않습니다.

마크다운은 `ConversationMarkdown`에서 렌더링합니다. 강조·목록·표·인용·링크·코드는
본문 서체와 데이터 서체를 따릅니다. HTML은 실행하지 않으며 이미지는 대체 텍스트만
표시합니다. 표와 코드는 내부에서 스크롤하고, 주석 참조는 메시지별로 구분합니다.
모바일에서는 캐릭터를 말풍선 머리글 안으로 옮겨 본문 너비를 확보합니다.
