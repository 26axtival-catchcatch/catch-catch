...
| --- | --- | --- |
| 오늘의 브리핑 카드 | 브리핑 조회 API가 없음 | 날짜, 시그널, 증감 지표, 추세, 근거 Source를 반환하는 브리핑 API 필요 |
| `지켜볼 것 요청하기` | 요청 생성/목록 API가 없음 | Watch 조건 생성·조회·상태 API 필요. 현재 UI는 로컬 시연 상태임 |
| Catching의 역할별 노드와 병렬 상태 | 공개 SSE는 Goal/Plan/Fact/Note와 공개 Fact 기록 단계만 제공 | coordinator/investigator/verifier/reporter의 시작·완료·실패, task/round/source를 공개하는 SSE가 필요. 내부 추론은 제외해야 함 |
| 역할별 실제 소요 시간 | `step_started/completed`는 공개 Fact 변환 단계 시간 | 역할 span의 공개 가능한 시작·완료·duration 필드가 필요. 현재 정확한 값은 Langfuse에서만 확인 가능 |
...
| 미확정 후보의 주장별 계보 | 후보와 사유가 `report.limitations` 문자열로만 제공 | candidate ID, statement, reason, fact/evidence refs를 가진 구조화된 후보 목록 필요 |
