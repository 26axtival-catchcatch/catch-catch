...
등록, 수동 측정, 일별 자동 실행과 비교 API가 준비되어 있습니다.
실행 버튼과 자동 분석 토글 연결은 [시그널 실행 트리거 FE 인계](signal-trigger-fe-handoff.md)를 먼저 확인합니다.
이 문서는 API 인계이며 FE 화면 구현은 포함하지 않습니다.
...
`measurement.trace_id`는 해당 측정을 최초로 저장한 조사, 수동 측정 또는 자동 측정의 Langfuse trace ID입니다.
...
분모 0도 비율 0%가 아닙니다. 해커톤 데이터의 특정 주간 밖에 기록이 없다면 실제 개선 추세로 설명하면 안 됩니다.
...
일별 스케줄러는 `status=active`이고 일정의 `enabled=true`인 시그널만 실행합니다.
등록 시 기본 일정을 생성하며 `PUT /api/signals/{signal_id}/schedule`로 설정합니다. 알림은 연결하지 않았습니다.
...
일별 자동 실행과 누적 비교 API는 구현되어 있습니다.
변화 추이 화면, LLM 정기 리포트와 알림은 후속 범위입니다.
...
추이 데이터는 `GET /api/signals/{id}/daily-results?limit=30`을 사용합니다.
