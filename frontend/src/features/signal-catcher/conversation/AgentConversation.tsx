...
function Message({ message }: { message: ConversationMessage }) {
  ...
  return (
    <li className={styles.message} data-kind={activity.kind} data-status={activity.status}>
      ...
      {activity.details.decisions.length > 0 || activity.details.limitations.length > 0 ? (
        <details className={styles.evidence}>
          ...
          {activity.details.decisions.map((decision, index) => (
            <p key={`${decision.candidate_id}:${index}`}>{decision.reason}</p>
          ))}
          {activity.details.limitations.map((limitation, index) => <p key={index}>{limitation}</p>)}
        </details>
      ) : null}
    </li>
  );
    ...
    <div className={styles.room}>
      <aside className={styles.sidebar} aria-label="참여 에이전트">
        <header className={styles.sidebarHeader}>
          <span>함께 분석하는 팀</span><b>{agents.length}</b>
        </header>
        <nav className={styles.participants} aria-label="에이전트별 대화 선택">
          <button type="button" className={styles.participant} aria-pressed={!activeAgent}
            ...
            <span className={styles.avatar} data-role="all" aria-hidden="true">✳</span>
            <span className={styles.participantText}><strong>전체 대화</strong><small>모든 에이전트의 분석 흐름</small></span>
            <span className={styles.messageCount}>{messages.length}</span>
          </button>
          {agents.map(agent => (
            <button key={agent.id} type="button" className={styles.participant}
              aria-pressed={activeAgent?.id === agent.id} onClick={() => selectAgent(agent.id)}>
              <span className={styles.avatar} data-role={agent.role} aria-hidden="true">{AGENT_ROLES[agent.role].mark}</span>
              <span className={styles.participantText}>
                <strong>{agent.label}</strong>
                <small>{agent.roundIndex > 0 ? `재조사 ${agent.roundIndex} · ` : ""}{ACTIVITY_STATUS[agent.status]}</small>
              </span>
              <span className={styles.statusDot} data-status={agent.status} data-halted={halted || completed} aria-hidden="true" />
            </button>
          ))}
        </nav>
        <p className={styles.sidebarNote}>단서를 찾고, 서로 확인하며<br />하나의 결론을 만들어 갑니다.</p>
      </aside>

      <section className={styles.thread} aria-label="분석 대화방">
        <header className={styles.threadHeader}>
          <div><h2>{activeAgent?.label ?? "에이전트 대화"}</h2>
            <p>{activeAgent ? AGENT_ROLES[activeAgent.role].description : "질문에서 결론까지, 에이전트가 남긴 기록"}</p></div>
        ...
        <div className={styles.viewport} ref={viewportRef} role="region" aria-label="에이전트 메시지" tabIndex={0}
          ...
          {groups.length ? groups.map((group, index) => {
            ...
            return <div key={group.messages[0].id}>
              ...
              <div className={styles.messageGroup}>
                ...
                <div className={styles.groupBody}>
                  <p className={styles.author}>{agent.label}{agent.roundIndex > 0 ? <small>재조사 {agent.roundIndex}</small> : null}</p>
            ...
            <p>{completed ? "이 분석에는 공개된 에이전트 메시지가 기록되지 않았어요." : "진행 설명과 결과 요약이 도착하면 시간순으로 보여드려요."}</p>
          </div>}
        </div>
