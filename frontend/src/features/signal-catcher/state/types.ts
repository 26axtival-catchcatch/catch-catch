...
/**
 * 진행 로그 한 줄의 종류. 로딩 화면의 레일이 이 값에 따라 다르게 그린다.
 * Backend Canonical Run Event 와 1:1 로 맞춰 둔다.
 */
export type TickKind = "think" | "tool" | "fact" | "reject";

/**
 * 분석 중 흘려보내는 진행 문장 하나.
 * `meta`는 그 문장이 어떤 이벤트 payload 자리에 대응하는지를 적어 둔 값이고,
 * 도구 호출일 때만 우측에 결과 요약으로 노출한다.
 */
export interface StageTick {
  kind: TickKind;
  /** 도구 호출일 때 실행한 primitive 이름. 나머지 종류는 null. */
  primitive: string | null;
  text: string;
  meta: string;
  /** 포착일 때 배지에 얹는 짧은 라벨. 나머지 종류는 null. */
  short: string | null;
  /** 이 문장이 화면에 머무는 시간. */
  ms: number;
}
...
export interface CatchReport {
  ...
  planSteps: PlanStep[];
  score: TraceScore;
  /** 이 리포트를 만든 Run. 처리 과정 트레이스를 외부 도구에서 열 때 쓴다. */
  runId: string;
