...
import type { CatchReport, JourneyNode } from "../state/types";
 ...
 * 노드를 누르면 곧바로 원본 근거로 간다. 사이에 상세 패널을 두지 않는다.
 */
export function JourneyFlow({ report, onOpenEvidence }: JourneyFlowProps) {
  const { journey, lanes } = report;
  const [cursor, setCursor] = useState(journey.length);
  const [playing, setPlaying] = useState(false);
  const [selectedId, setSelectedId] = useState<string>(
    journey[journey.length - 1]?.event_id ?? "",
  );
  ...
  return (
    <div className={styles.wrap}>
                  ...
                  return (
                    <div key={`${lane.id}-${node.event_id}`} className={styles.cell}>
                      <button
                        ...
                        aria-label={`${lane.label} · ${node.action} · ${node.text} · 원본 근거 열기`}
                        title={node.text}
                        onClick={() => {
                          setPlaying(false);
                          setSelectedId(node.event_id);
                          if (index >= cursor) setCursor(index + 1);
                          onOpenEvidence(node.evidence_id);
                        }}
                      >
      ...
      {/* 재생하는 동안 지금 어느 지점인지 한 줄로만 읊어 준다. 패널은 두지 않는다. */}
      {selected ? (
        <p className={styles.caption} aria-live="polite">
          ...
          <span className={styles.captionInsight}>{selected.insight}</span>
        </p>
      ) : null}
    </div>
  );
